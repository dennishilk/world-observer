...
- Trend window: 180 minutes. WSV requests that short window directly; NLWKN requests the documented recent response and then filters parsed measurements to the 180-minute window ending at the selected latest measurement timestamp.
...
The adapter fails closed unless station `184`, parameter `1`, and pinned Datenspur `144222103` validate against the pinned official metadata; other Datenspuren are kept only as concise diagnostics and are never merged into the parsed measurements. Live diagnostics showed `144222103` as the physical `Wasserstand` measurement trace (`IstWasserstand=true`, `HatPegelstaende=true`, actual current values) and `144316942` as a non-selected tide/reference trace (`mittlere Tidekurve`/`IstWasserstand=false`), so only `144222103` is parsed. It logs the raw measurement timestamp string exactly as returned before parsing in `diagnostics.raw_measurement_timestamp`; when `DatumUTC` is present it also logs the exact raw `DatumUTC` value and Python type before parsing in `diagnostics.raw_measurement_datum_utc` and `diagnostics.raw_measurement_datum_utc_type`. The preferred timestamp source is the confirmed official NLWKN `DatumUTC` Microsoft JSON date representation `/Date(<epoch_ms>)/`, with `/Date(<epoch_ms><offset>)/` also accepted when an offset is present; both forms are normalized to timezone-aware UTC from the epoch milliseconds. The local `DD.MM.YYYY HH:MM` timestamp is retained only as a comparison/fallback diagnostic when `DatumUTC` exists. Without `DatumUTC`, it accepts the confirmed official German local timestamp format `DD.MM.YYYY HH:MM` interpreted with `Europe/Berlin` zoneinfo rules, plus Microsoft JSON date strings with an explicit numeric offset, for example `/Date(1783786800000+0200)/` where used elsewhere. Parsed timestamps are normalized to UTC for emitted observations. Future measurements are not marked fresh; a small configured clock-skew tolerance is accepted but exposed as diagnostics with a non-negative public age, while timestamps beyond that tolerance fail closed. Malformed timestamps, malformed `DatumUTC` values, duplicate timestamps, non-numeric values, and changed pinned metadata keep the NLWKN adapter unavailable rather than silently dropping records.
