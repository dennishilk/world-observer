...
def test_no_solar_wind_sources_configured_by_default() -> None:
    assert observer.SOURCES["kp"] == "https://services.swpc.noaa.gov/products/noaa-planetary-k-index.json"
    assert "mag" not in observer.SOURCES
    assert "plasma" not in observer.SOURCES
    assert observer.SOURCE_CANDIDATES == {"mag": [], "plasma": []}
...
def test_kp_available_with_header_only_mag_plasma_is_partial(monkeypatch) -> None:
    ...
    def fake_fetch(url: str, timeout_s: int = observer.TIMEOUT_S):
        ...
        raise AssertionError(f"unexpected solar-wind fetch {url}")
    ...
    assert built["diagnostics"]["solar_wind_source_status"] == "not_configured"
    assert built["solar_wind"] == {
        "bz_gsm": None,
        "bz_gsm_observed_at_utc": None,
        "speed_km_s": None,
        "speed_observed_at_utc": None,
    }
